...
function initCursor() {
  ...
  document.addEventListener('mousedown', () => { ring.style.width = '14px'; ring.style.height = '14px'; });
  document.addEventListener('mouseup',   () => { ring.style.width = '28px'; ring.style.height = '28px'; });
  const raf = () => { rx += (tx - rx) * .12; ry += (ty - ry) * .12; ring.style.left = rx + 'px'; ring.style.top = ry + 'px'; requestAnimationFrame(raf); };
